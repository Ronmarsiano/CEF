...
def check_red_hat_firewall_issue():
    print("Checking if firewalld is installed.")
...
def red_hat_firewall_d_exception_for_omsagent():
    ...
    return "127.0.0.1\32" in o and agent_port in o
...
def test_daemon_configuration(daemon_name):
    ...
    if not is_daemon_dir_exists:
        print_error("Error: Could not locate " + daemon_name + " directory.[under \'/etc/\']")
        return False
    print_ok("Ok: Located /etc/ "+ daemon_name + " directory.")
    print("Checking oms configuration under the name of: \'security-config-omsagent.conf\'")
    config_exists = check_file_in_directory("security-config-omsagent.conf", "/etc/" + daemon_name + "/")
    if not config_exists:
        print_error("Error: security-config-omsagent.conf does not exists in " + daemon_name + " directory")
        return False
    else:
        print_ok("Ok: Located security-config-omsagent.conf")
        return True
...
def validate_daemon_configuration_content(daemon_name, valid_content_tokens_arr):
    ...
    if not file_contains_string(valid_content_tokens_arr, "/etc/" + daemon_name + "/security-config-omsagent.conf"):
        ...
        print_ok("Ok: security-config-omsagent.conf contains " + daemon_name + " routing configuration")
        return True
...
def security_config_omsagent_test(workspace_id):
    ...
    if not is_security_config_omsagent_dir_exists:
        ...
        print_ok("Ok: Located security_events.conf")
        return True
...
def omsagent_security_event_conf_validation(workspace_id):
    ...
    if not sudo_read_file_contains_string(file_tokens=oms_agent_configuration_content_tokens, file_path=path):
        print_error("Error: could not locate necessary port and ip in the agent's configuration.\npath:"+path)
    else:
        print_ok("Ok: omsagent event configuration is valid")
    ...
    if len(tokens) > 1:
        for single_token in tokens:
            if "/usr/sbin/" + daemon_name in single_token:
                print_ok("Ok: Found " + daemon_name + " process running on this machine.")
                return True
...
def restart_daemon(daemon_name):
    ...
    if e is not None:
        ...
        print_ok("Ok: " + daemon_name + " daemon restarted.")
...
def restart_omsagent(workspace_id):
    ...
    if e is not None:
        ...
        print_ok("Ok: omsagent restarted.")
        print("This will take 5 seconds.")
        time.sleep(5)
...
def handle_syslog_ng(workspace_id):
    ...
    if test_daemon_configuration("syslog-ng"):
        ...
        if daemon_config_valid:
            print_ok("Ok: syslog-ng daemon configuration was found valid.")
...
def handle_rsyslog(workspace_id):
    ...
    if test_daemon_configuration("rsyslog.d"):
        print_ok(
            "Ok - rsyslog daemon found, checking daemon configuration content - forwarding facility local-4 to port " + daemon_port)
        ...
        if not daemon_config_valid:
            ...
            print_ok("Ok: rsyslog daemon configuration was found valid.")
        print("Trying to restart syslog daemon")
